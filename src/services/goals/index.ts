...
export async function createGoal(userId: bigint, input: CreateGoalInput) {
  ...
  if (input.allocations?.length) {
    const totalPct = input.allocations.reduce(
...
export async function getActiveGoal(userId: bigint) {
  ...
  const currentPrincipal = assets.reduce(
    (sum, a) => sum + (a.costAmount ? Number(a.costAmount) : 0),
    0,
  );
...
export async function updateGoal(
  ...
  if (input.targetAmount != null)
    data.targetAmount = new Decimal(input.targetAmount);
  if (input.targetDate != null) data.targetDate = new Date(input.targetDate);
  if (input.initialPrincipal != null)
    data.initialPrincipal = new Decimal(input.initialPrincipal);
...
function serializeGoalWithAllocations(goal: Record<string, unknown>) {
  ...
  const allocations =
    (goal.allocations as Record<string, unknown>[] | undefined) ?? [];
  return {
    ...base,
