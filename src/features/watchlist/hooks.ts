...
export function useWatchlist() {
  return useQuery({
    queryKey: ["watchlist"],
    queryFn: () =>
      apiFetch<{ items: WatchlistItem[] }>("/api/watchlists").then(
        (r) => r.items,
      ),
  });
}
  ...
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<SearchResult[]>(
        `/api/market-data/search?keyword=${encodeURIComponent(keyword)}`,
      ),
...
export function useRemoveWatchlist() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      apiFetch<null>(`/api/watchlists/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["watchlist"] });
